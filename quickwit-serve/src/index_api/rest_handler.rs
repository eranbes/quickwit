...
pub fn index_management_handlers(
    index_service_opt: Option<Arc<IndexService>>,
) -> impl Filter<Extract = impl warp::Reply, Error = Rejection> + Clone {
    get_index_metadata_handler(index_service_opt.clone())
        .or(get_indexes_metadatas_handler(index_service_opt.clone()))
        .or(create_index_handler(index_service_opt.clone()))
        .or(delete_index_handler(index_service_opt))
}
...
mod tests {
    ...
    async fn test_rest_get_index() -> anyhow::Result<()> {
        ...
        let index_management_handler =
            super::index_management_handlers(Some(Arc::new(index_service))).recover(recover_fn);
    ...
    async fn test_rest_create_index() -> anyhow::Result<()> {
        ...
        let index_management_handler =
            super::index_management_handlers(Some(Arc::new(index_service)));
        let resp = warp::test::request()
            .path("/indexes")
            .method("POST")
            .json(&true)
    ...
    async fn test_rest_create_index_with_bad_config() -> anyhow::Result<()> {
        ...
        // TODO: update test when recover_fn will handle correctly this kind of errors.
        Ok(())
    }
}
